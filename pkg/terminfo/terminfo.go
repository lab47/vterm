...
func (pb *paramsBuffer) EndBW() error {
	err := pb.bw.Flush()
	pb.lk.Unlock()
	return err
}
...
func (t *Terminfo) parmf(w ByteWriter, pb *paramsBuffer, s string, p ...int) {
	var a, b string
	var ai, bi int
	var ab bool
	var dvars [26]string
	...
	for {
		...
		case '0', '1', '2', '3', '4', 'x', 'X', 'o', ':':
			...
			case 'd', 'x', 'X', 'o':
				ai, stk = stk.PopInt()
				w.WriteString(fmt.Sprintf(f, ai))
			case 'c', 's':
				a, stk = stk.Pop()
				w.WriteString(fmt.Sprintf(f, a))
			}
...
// TPuts emits the expansion of the givine param to the writer, but expands inline padding
...
func (t *Terminfo) TParmf(w io.Writer, s string, p ...int) error {
	beg := strings.Index(s, "$<")
	if beg >= 0 {
		t.TPuts(w, t.TParm(s, p...))
		return nil
	}
	...
	if pb.bw == nil {
		pb.bw = bufio.NewWriter(w)
	} else {
		pb.bw.Reset(w)
	}

	t.parmf(pb.bw, pb, s, p...)
